...
For a quantum state $\rho$ on a Hilbert space $\mathcal{H}$, the Ω-ergotropy with respect to a Hamiltonian $H$ and parameter $\Omega \in \mathbb{R}$ is defined as
\begin{equation}
...
\end{equation}
where $\mathcal{U}(\mathcal{H})$ is the group of unitary operators and $\mathcal{S}$ denotes the von Neumann entropy.
\end{definition}
